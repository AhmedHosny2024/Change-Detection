...
import imageio
# Mean Jaccard Index: 0.05633107795687511
...
def gene_block(img, block_sz=4, gene_block_vec=True):
    ...
    if gene_block_vec:
        ...
        left_pad = np.int64(np.ceil(block_sz / 2)) - 1
        right_pad = block_sz - np.int64(np.ceil(block_sz / 2))
        up_pad = np.int64(np.ceil(block_sz / 2)) - 1
        down_pad = block_sz - np.int64(np.ceil(block_sz / 2))
        ...
        for y in range(up_pad, up_pad + n_y):
            for x in range(left_pad, left_pad + n_x):
                vert_start = y - np.int64(np.ceil(block_sz / 2)) + 1
                vert_end = y + block_sz - np.int64(np.ceil(block_sz / 2)) + 1
                horiz_start = x - np.int64(np.ceil(block_sz / 2)) + 1
                horiz_end = x + block_sz - np.int64(np.ceil(block_sz / 2)) + 1
...
def pca_k_means(img_diff, block_size=4, eig_space_dim=3):
    ...
    cluster_center = KMeans(n_clusters=2,n_init=10).fit(feature_vec.T).cluster_centers_.T  # shape: (S, 2)

    # generate change map
    change_map = gene_change_map(img_diff, feature_vec, cluster_center)
    return change_map

def pca_kmeans_change(images_A,images_B):
    results = []
    eig_dim = 10
    block_sz = 4
    c=0
    for i in range(len(images_A)):
        print(c)
        diff_img = diff_image(images_A[i], images_B[i], is_abs=True, is_multi_channel=True)
        change_img = pca_k_means(diff_img, block_size=block_sz,
                                 eig_space_dim=eig_dim)
        results.append(change_img)
        c+=1
    return results

def main():
    
    before_img = cv2.imread('E:/College/Fourth Year/second term/RSSI/Project/Change-Detection/trainval/A/0069.png')[:, :, 0:3]
    ...
    change_img = pca_k_means(diff_img, block_size=block_sz,
                             eig_space_dim=eig_dim)
    cv2.imwrite('change.png', change_img)
